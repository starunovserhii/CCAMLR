...
const heroBanner = `
<div class="hero-banner" role="img" aria-label="Стилізована ілюстрація: судно ярусного лову серед айсбергів моря Росса під зоряним антарктичним небом, риба-клич під водою, пінгвіни на кризі">
...
${table(["Ресурс","Навіщо спостерігачу"], [
  ...
  ["<a href='https://ecds.ccamlr.org' target='_blank' rel='noopener'>e-CDS (Catch Documentation Scheme)</a>","Електронна система документації вилову клича — допомагає зрозуміти, куди йдуть дані з форми C2"],
...
},
{
  ...
  ua: `<p>Репрезентативну вибірку риби слід брати з кожної виборки для фіксації біологічних характеристик (довжина, вага, стать тощо). Вимоги до відбору проб клича описані на сторінці вимог до вибірки спостерігача на сайті CCAMLR.</p>
  <p>Орієнтовна норма відбору проб клича — приблизно 7 риб на 1000 гачків, або загалом 35 риб на лінію (за умови середньої лінії на 5000 гачків). З цих 35 риб спостерігачі мають відібрати 10 риб на лінію з повним записом виду, загальної довжини, статі, стадії зрілості гонад, загальної ваги та відбором 10 отолітів; і 25 риб на лінію лише з біологічними промірами (без відбору отолітів). Норми відбору базуються на середній кількості гачків на постановку. Якщо судна ставлять коротші лінії, з'єднані «join line» (наприклад, щоб зменшити кількість вертикальних лінок і буїв на поверхні, які може захопити лід), тепер їх потрібно звітувати як одну виборку, тому вимоги до відбору проб також слід застосовувати до з'єднаних коротких ліній як до єдиного суцільного знаряддя лову.</p>
  ...
  extra: gallery([
    ["images/logbook/lb_003.jpg","Рис. 1. Вимір клича: стандартна (SL) і загальна (TL) довжина"],
...
section("similar-species", "6.9 Порівняльна таблиця схожих видів", `
  ...
  ${h3("Клич: Dissostichus mawsoni (TOA) проти D. eleginoides (TOP)")}
  ${gallery([["images/rossfish/ross_035.jpg","D. mawsoni (TOA) — антарктичний клич"],["images/rossfish/ross_033.jpg","D. eleginoides (TOP) — патагонський клич"]], "wide")}
...
["CDS", "[сі-ді-е́с]", "Catch Documentation Scheme", "Схема документування вилову клича."],
...
["COLTO", "[ко́лтоу]", "Coalition of Legal Toothfish Operators", "Коаліція легальних операторів промислу клича — пропонує приз за знайдені мітки."],
...
["Maturity stage", "[мет'ю́риті стейдж]", "Стадія зрілості", "Класифікація ступеня розвитку гонад (наприклад, F1–F5 для самок клича)."],
...
// July 2026). Site так само як і в інших розділах фокусується на клачі —
...
const page17 = section("krill", "17.1 Промисел антарктичного криля (Euphausia superba)", `
  ${p("Криль — наймасовіший за обсягом вилову промисел зони ККАМЛР, суттєво відмінний від ярусного промислу клача логістикою обліку й документообігом спостерігача.")}
  ...
  ${p("<strong>Для спостерігача:</strong> обсяги вилову й темп обробки на промислі криля значно вищі, ніж на ярусному лові клача — обробка часто безперервна. Форми обліку прилову й біологічного відбору проб для криля відрізняються від форми C2 (яка розрахована на ярусний промисел); ознайомтеся з формою, специфічною для тралового промислу криля, до початку рейсу — уточнюйте в технічного координатора, яка саме форма застосовується на вашому судні.")}
`) +
section("squid", "17.2 Промисел кальмарів (Martialia hyadesi)", `
  ...
  ${ul([
    ...
    "Кальмар (зокрема гігантські екземпляри) частіше трапляється спостерігачу як <strong>прилов</strong> на ярусному промислі клача, ніж як цільовий об'єкт промислу."
  ])}
...
section("insp-port", "18.3 Портова інспекція (CM 10-03)", `
  ${table(["Вимога", "Деталі"], [
    ["Судна з кличем на борту", "Обов'язкова інспекція 100% таких суден при заході в порт"],
    ...
    ["Перевірка документації", "Вивантаження/перевантаження клача звіряється з документацією Catch Documentation Scheme (CDS)"]
  ])}
`) +
...
function progRow([topic, where]) {
  return [topic, where];
}

const page19 = section("intern-intro", "Про програму стажування", `
  ${p("Ця сторінка відтворює розклад тем 3-денного очного стажування для кандидатів на посаду наукового спостерігача ККАМЛР і показує, який розділ цього сайту покриває кожну тему — щоб зорієнтуватися, що вже опрацьовано, а що варто уточнити додатково.")}
  ${comment("Розклад складено на основі внутрішньої програми стажування (не офіційний документ CCAMLR). Деякі теми (законодавство України, внутрішня звітність до інституту, взаємодія з екіпажем) виходять за межі тематики цього сайту — там, де сайт не дає покриття, це позначено окремо.")}
`) +
section("intern-day1", "День перший", `
  ${h3("Загальні питання")}
  ${table(["Тема заняття", "Розділ сайту"], [
    progRow(["Зона ККАМЛР. Географічне розташування. Історія створення.", "<a href=\"01-governing.html\">Керівні документи</a>"]),
    progRow(["Договірні сторони — учасники ККАМЛР", "<a href=\"01-governing.html\">Керівні документи</a>"]),
    progRow(["Керівні документи ККАМЛР", "<a href=\"01-governing.html\">Керівні документи</a>, <a href=\"13-library.html\">Бібліотека документів</a>"]),
    progRow(["Правила ККАМЛР та Українське законодавство", "— (поза межами сайту; законодавство України тут не висвітлюється)"]),
    progRow(["Заходи зі збереження ККАМЛР. Класифікація та процедура змін.", "<a href=\"02-measures.html\">Заходи зі збереження</a>"]),
    progRow(["Термінологія і коди ККАМЛР", "<a href=\"14-glossary.html\">Глосарій термінів</a>, <a href=\"15-flashcards.html\">Флеш-картки</a>"])
  ])}
  ${h3("Географічні, океанологічні та біологічні особливості регіону. Знаряддя лову")}
  ${table(["Тема заняття", "Розділ сайту"], [
    progRow(["Підрозділи, ділянки та дослідницькі квадрати регіону", "<a href=\"02-measures.html\">Заходи зі збереження</a> (нумерація підрайонів)"]),
    progRow(["Океанологічні особливості Південного океану", "— (поза межами сайту, лише побіжно в описах промислів)"]),
    progRow(["Головні промислові об'єкти зони ККАМЛР", "<a href=\"06-identification.html\">Визначники</a> (клач), <a href=\"17-other-fisheries.html\">Інші промисли</a> (криль, кальмари, краби)"]),
    progRow(["Знаряддя лову й промисел клача, криля, кальмарів, крабоідів", "<a href=\"05-gear.html\">Знаряддя лова</a>, <a href=\"17-other-fisheries.html\">Інші промисли</a>"]),
    progRow(["Правила регулювання промислу в зоні ККАМЛР", "<a href=\"02-measures.html\">Заходи зі збереження</a>"]),
    progRow(["Регулювання промислу для окремих об'єктів", "<a href=\"02-measures.html\">Заходи зі збереження</a>, <a href=\"17-other-fisheries.html\">Інші промисли</a>"])
  ])}
`) +
section("intern-day2", "День другий — Науковий спостерігач: права, обов'язки, завдання", `
  ${table(["Тема заняття", "Розділ сайту"], [
    progRow(["Національні та міжнародні спостерігачі", "<a href=\"01-governing.html\">Керівні документи</a> (Схема SISO)"]),
    progRow(["Права та обов'язки спостерігачів", "<a href=\"01-governing.html\">Керівні документи</a>"]),
    progRow(["Облік цільових видів і прилову (клач, криль, кальмари, крабоіди)", "<a href=\"04-forms-instructions.html\">Форми та інструкції</a>, <a href=\"17-other-fisheries.html\">Інші промисли</a>"]),
    progRow(["Аналіз клача: стадії зрілості гонад, гістологічні дані", "<a href=\"06-identification.html\">Визначники</a> (частково — детальний гістологічний протокол не описано)"]),
    progRow(["Збір і зберігання отолітів для визначення віку", "<a href=\"04-forms-instructions.html\">Форми та інструкції</a>, <a href=\"14-glossary.html\">Глосарій</a> (термін «Otolith»)"]),
    progRow(["Величина улову й переводні коефіцієнти", "<a href=\"04-forms-instructions.html\">Форми та інструкції</a>, <a href=\"14-glossary.html\">Глосарій</a>"]),
    progRow(["Методика й норми мічення клача", "<a href=\"08-tagging.html\">Мічення (Tagging)</a>"]),
    progRow(["Підрахунок VME-індикаторних організмів", "<a href=\"07-vme.html\">VME</a>"]),
    progRow(["Спостереження за птахами й ссавцями; прилов гігантських кальмарів", "<a href=\"04-forms-instructions.html\">Форми та інструкції</a> (IMAF), <a href=\"17-other-fisheries.html\">Інші промисли</a> (кальмари)"]),
    progRow(["Звітність про ННН-лов і забруднення моря", "<a href=\"11-templates.html\">Заповнення журналів</a> (форма IUU sighting), <a href=\"18-inspection.html\">Інспекційна система</a>"]),
    progRow(["Наукові лови, збір планктону, прилад DST CTD", "— (поза межами сайту, детального протоколу немає)"]),
    progRow(["Особисті пристрої спостерігача (ноутбук, фотоапарат, GoPro)", "<a href=\"16-checklist.html\">Чек-лист підготовки</a>"]),
    progRow(["Наукові спостереження на промислі криля", "<a href=\"17-other-fisheries.html\">Інші промисли</a>"]),
    progRow(["Звітність судна до Секретаріату ККАМЛР", "<a href=\"04-forms-instructions.html\">Форми та інструкції</a> (Cruise Report)"]),
    progRow(["Звітність до інституту (терміни, форма)", "— (поза межами сайту — внутрішня процедура вашої організації)"]),
    progRow(["Взаємодія з екіпажем, комсоставом, капітаном, іншим спостерігачем", "— (поза межами сайту — практичні/комунікативні навички)"])
  ])}
`) +
section("intern-day3", "День третій — Інспекційна система. Іспит", `
  ${table(["Тема заняття", "Розділ сайту"], [
    progRow(["Інспекційна перевірка суден у морі", "<a href=\"18-inspection.html\">Інспекційна система</a>"]),
    progRow(["Інспекційна перевірка суден у порту", "<a href=\"18-inspection.html\">Інспекційна система</a>"]),
    progRow(["Взаємодія наукового спостерігача з інспекторами", "<a href=\"18-inspection.html\">Інспекційна система</a>"]),
    progRow(["Іспит для кандидатів", "<a href=\"10-test.html\">Самоперевірка (тест)</a> — режим екзамену з таймером і підсумком за розділами"])
  ])}
`);
