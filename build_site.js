...
["POG","Pogonophryne spp","Плундерфіш (бородаті вудильники), nei"],
["ANT","Antimora rostrata","Синя антимора"],
["LIZ","— не знайдено в довіднику —","На фото — вид, схожий на батіподиду; потребує підтвердження"],
["SRR","Amblyraja georgiana","Антарктичний зірчастий скат"]
];
...
section("riba", "6.6 Довідка з фотографіями видів і кодами (риба.doc)", `
  ...
  ${comment("10 з 11 кодів підтверджено: 9 — офіційним довідником CCAMLR (аркуш «CCAMLR codes» у en_C2_2026a.xlsx), а код <strong>ELZ</strong> додатково зіставлено з визначником Ross Sea Fishes (розділ 6.5, зображення сторінки 31) — це родина Zoarcidae (бельдюгові). Код <strong>LIZ</strong> залишається непідтвердженим: на фото — вид, схожий на батіподиду, але точного збігу в жодному з наданих довідників не знайдено. Перед використанням коду LIZ у офіційній звітності рекомендую звірити з повним переліком видових кодів CCAMLR на сайті ccamlr.org.")}
`) +
...
const page07 = section("vme-guide", "7.1 CCAMLR VME Taxa Classification Guide 2023, версія 2", `
  ...
  ${p("Офіційний візуальний визначник таксонів-індикаторів вразливих морських екосистем (Vulnerable Marine Ecosystems). Складається з 4 сторінок-таблиць «Тип → Код → Рівень → Таксон» з описом форми, розміру, текстури та типових помилок ідентифікації. Наведено повністю нижче.")}
  ${gallery([
    ...
    ["images/vme/vme_p-3.jpg","Сторінка 3: продовження визначника"],
    ["images/vme/vme_p-4.jpg","Сторінка 4: продовження визначника"]
  ], "wide")}
`) +
section("vme-terms", "7.2 Ключові терміни та вимоги", `
  ${ul([
...
${section("cs6", "6. Визначники", ul([
  ...
  "<strong>риба.doc — коди:</strong> TOA, GRV, CHW, ICX, TRT, MRL, POG, ANT, SRR та ELZ (=Zoarcidae, за визначником Ross Sea Fishes) підтверджені; LIZ — не знайдено, потребує перевірки."
]))}
${section("cs7", "7. VME", ul([
  "<strong>Групи-індикатори:</strong> Cnidaria (горгонарії, чорні/кам'янисті корали, гідроїди, зоантарії), Porifera (губки), Chordata (асцидії), Bryozoa (мохуватки).",
...
const goldenRules = [
  ...
  "Формат дати: <strong>ДД/ММ/РРРР</strong> (або як задано в шаблоні аркуша — не змінювати формат клітинки вручну).",
  "Порожніх обов'язкових клітинок бути не повинно: якщо значення відсутнє (наприклад, вид не визначено), використовуйте узгоджений код («невідомо»/«nei»), а не залишайте клітинку порожньою.",
  "<strong>Set/Haul ID</strong> — наскрізний унікальний номер, однаковий на всіх аркушах, що стосуються цієї виборки (Set and Haul Details, Observed Haul Catch, Haul IMAF, Haul VME, Biological Sampling, Tagging). Розбіжність ID між аркушами — найчастіша причина повернення журналу на доопрацювання.",
  "Десятковий роздільник — крапка (<code>12.5</code>), не кома.",
  "Видові коди — трилітерні коди CCAMLR (аркуш «CCAMLR codes» у en_C2_2026a.xlsx), не довільні скорочення.",
  "Координати — у десяткових градусах або градусах/хвилинах відповідно до формату шаблону; південна широта та східна/західна довгота — з правильним знаком/літерою півкулі.",
  "Заповнювати впродовж виборки/дня, а не «заднім числом» наприкінці рейсу — це головна причина помилок і забутих деталей.",
  "Перед відправкою — звірити електронний журнал з паперовими нотатками і зі звітом про рейс (розділ 3.2) на предмет узгодженості цифр."
];

const templVesselGear = [
  ["Номер IMO судна","9123456","Постійний номер судна, не плутати з позивним"],
  ["Назва судна","F/V Example Star","Точна назва з реєстрації, латиницею як у нотифікації"],
  ["Позивний (Call sign)","ABCD","Міжнародний радіопозивний"],
  ["Тип знаряддя","Autoline","Autoline / Spanish line / Trotline / Pot — обрати зі списку, звірити з нотифікацією CCAMLR"],
  ["Висота кріплення стример-лінії","6.2 м","Вимірюється від точки виходу лінії до поверхні води (див. рис. 1, розділ 4.1)"],
  ["Крок кріплення стримерів","5 м","Відповідно до CM 25-02 Annex A — залежить від довжини лінії"],
  ["Пристрій виключення ссавців","Mesh panel","Вказати конкретний тип, встановлений на цьому рейсі, а не типовий з довідника"]
];

const templSetHaul = [
  ["Set/Haul ID","2026-014-H032","Єдиний наскрізний номер — використовується на ВСІХ пов'язаних аркушах без винятку"],
  ["Дата/час початку постановки (UTC)","14/07/2026 03:15","UTC, не судновий час"],
  ["Дата/час завершення виборки (UTC)","14/07/2026 09:40","Має бути пізніше за час постановки"],
  ["Широта/довгота початку","65°12.4′S, 179°58.1′W","Формат згідно з шаблоном; перевірити півкулю"],
  ["Кількість гачків / довжина лінії","3800 гачків / 4.5 км","Узгодити з декларованим типом знаряддя"],
  ["Глибина","1120 м","В метрах, середня глибина сегмента"],
  ["Підрайон/поділ","88.1","Відповідно до статистичних районів CCAMLR (розділ 3.3)"]
];

const templObsCatch = [
  ["Обліковий період (% виборки)","25%","Мінімум рекомендований — не занижувати без обґрунтування"],
  ["Код виду","TOA","Трилітерний код CCAMLR, звірити з довідником при сумніві (розділ 6.6)"],
  ["Кількість екземплярів","47","Ціле число за обліковий період"],
  ["Загальна (зелена) вага","310 кг","В кілограмах, до переробки"],
  ["Ознаки хижацтва (голови/губи)","3 з 47","Окремий підрахунок голів/губ з ознаками депредації — не змішувати із загальним виловом"]
];

const templHaulImaf = [
  ["Set/Haul ID","2026-014-H032","Має збігатися з аркушем Set and Haul Details"],
  ["Вид птаха/ссавця","Diomedea exulans (мандрівний альбатрос)","Латинська назва або код CCAMLR"],
  ["Стан на момент вилову","Мертвий","Живий (відпущений) / Живий (травмований) / Мертвий"],
  ["Спостережуваність виборки","Так, спостерігач присутній","Вказати чи виборка була в обліковому періоді спостерігача"],
  ["Захід пом'якшення застосовано","Стример-лінія, нічна постановка","Перелічити фактично застосовані заходи на момент інциденту"]
];

const templMarineMammal = [
  ["Set/Haul ID","2026-014-H032","Той самий обліковий період, що й для Observed Haul Catch"],
  ["Вид","Тюлень-крабоїд (Lobodon carcinophagus)","Використовувати визначник, розділ 6.7, за наявності сумніву — фото"],
  ["Чисельність (мін./макс.)","2 / 4","Оцінка діапазону, якщо точний підрахунок неможливий"],
  ["Ознаки депредації","Так — вилучена рибка з гачка без риби","Фіксувати навіть непрямі ознаки (пошкоджений улов)"]
];

const templHaulVme = [
  ["Set/Haul ID","2026-014-H032","Той самий сегмент лінії 1000 гачків/1200 м, що описаний у розділі 7.2"],
  ["Тип вибірки","Trigger","Random (випадкова ~30%) або Trigger (≥5 одиниць-індикаторів)"],
  ["Таксон-індикатор","Cnidaria — горгонарії","Код відповідно до визначника VME (розділ 7.1)"],
  ["Об'єм/вага","1.5 л","У літрах (10-л контейнер) або в кг для гіллястих форм"],
  ["Кількість одиниць-індикаторів","6","≥5 на сегмент — автоматично запускає Trigger sample"]
];

const templBioSampling = [
  ["Set/Haul ID","2026-014-H032","Узгодити з Set and Haul Details"],
  ["Код виду","TOA","Трилітерний код CCAMLR"],
  ["Стандартна довжина (SL)","98 см","Вимір згідно з рис. розділу 4.1 — від писка до основи хвостового плавця"],
  ["Загальна довжина (TL)","106 см","Від писка до кінця хвостового плавця"],
  ["Стать","F (самиця)","M/F/невизначено"],
  ["Стадія зрілості","F3","За шкалою F1–F5/M1–M5, розділ 6.2"],
  ["Отоліти відібрано","Так","Ціль — 10 риб з отолітами на кожні 35 біологічних проб"],
  ["Норма відбору за лінію","35 риб (7 на 1000 гачків)","Мінімальна норма — не занижувати навіть при малому улові"]
];

const templConversion = [
  ["Дата тесту","16/07/2026","Повторювати щотижня для кожного району управління"],
  ["Код виду","TOA","Один тест на вид на район управління за тиждень"],
  ["Кількість риб у тесті","22","Не менше 20 риб — мінімальна вимога"],
  ["Загальна (зелена) вага","187 кг","До переробки"],
  ["Вага продукту","121 кг","Після переробки (обраний тип продукту)"],
  ["Розрахований коефіцієнт","1.55","= зелена вага / вага продукту"]
];

const templTagging = [
  ["Set/Haul ID","2026-014-H032","Узгодити з виборкою, де рибу підняли на борт"],
  ["Номер мітки (основна)","CCAMLR 123456","Послідовний номер з партії; перевірити на дублікати (умовне форматування)"],
  ["Номер мітки (друга, за наявності)","CCAMLR 123457","Подвійне мічення послідовними номерами — рекомендована практика"],
  ["Код виду","TOA","Клич або скат — окремі критерії придатності (розділ 8.1)"],
  ["Довжина на момент мічення","91 см","Вимірюється до мічення"],
  ["Загальний час поза водою","2 хв 40 с","Має не перевищувати 3 хвилини — критична вимога"],
  ["Хто виконав мічення","Спостерігач","Спостерігач / навчений член екіпажу"]
];

const templRecapture = [
  ["Номер мітки","CCAMLR 098765","Списати точно з фізичної мітки або фото"],
  ["Фото мітки на шаблоні-лінійці","Додано (файл IMG_0231.jpg)","Обов'язково — фізичну мітку повертати вже не потрібно, лише якісне фото"],
  ["Довжина на момент повторного вилову","104 см","Порівняти з довжиною при первинному міченні, якщо відома"],
  ["Стать і стадія зрілості","F, F4","Повний біологічний відбір проб типу III"],
  ["Дата/координати повторного вилову","19/07/2026, 65°08′S 178°40′W","UTC і формат координат як в інших аркушах"]
];

const templWaste = [
  ["Дата","15/07/2026","—"],
  ["Тип відходу","Уламок ярусної снасті","Знаряддя лову / побутові відходи / інше"],
  ["Причина втрати/скидання","Обрив лінії під час шторму","Випадково / навмисно — вказати чесно, це нормативна вимога, а не «звинувачення»"],
  ["Частота","Нерегулярно","Нерегулярно / щотижня / щодня — залежно від фактичної картини за рейс"]
];

const templIuu = [
  ["Дата/час спостереження (UTC)","17/07/2026 14:20","—"],
  ["Координати","64°55′S, 177°12′W","—"],
  ["Опис судна/знаряддя","Судно без розпізнавальних знаків, ярусна снасть без буїв","Максимально докладно: колір корпусу, надбудова, наявність прапора"],
  ["Дії спостерігача","Фото зроблено, координати зафіксовано, повідомлено капітану","Не вступати в контакт із судном; лише фіксація"]
];

const page11 =
section("templ-intro", "11.0 Загальні правила заповнення (перед усіма аркушами)", `
  ${p("Нижче — приклади заповнення для всіх 12 робочих аркушів електронного журналу спостерігача (en_C2_2026a.xlsx, розділ 4.1). <strong>Усі значення в колонці «Приклад заповнення» — вигадані навчальні дані</strong> (умовне судно, умовний рейс 2026 року), призначені лише для ілюстрації формату й логіки заповнення. Це не реальні дані жодного судна чи рейсу.")}
  ${ul(goldenRules)}
  ${comment("Цей розділ складено на основі вимог, розсіяних по кількох документах (2026 Observer Longline Logbook Instructions, C2 Commercial data manual, VME guide) і зведено в один практичний чекліст — у жодному з оригінальних файлів такого зведеного переліку немає.")}
`) +
sheetTemplate("templ-vessel", "11.1 Vessel and Gear", "en_C2_2026a.xlsx", templVesselGear,
...
sheetTemplate("templ-sethaul", "11.2 Set and Haul Details", "en_C2_2026a.xlsx", templSetHaul,
  "Це «хребет» журналу — Set/Haul ID звідси переноситься практично на всі інші аркуші. Типова помилка — розбіжність в один символ (наприклад, H032 на одному аркуші та H32 на іншому), через що аркуші перестають зв'язуватися між собою.") +
sheetTemplate("templ-obscatch", "11.3 Observed Haul Catch", "en_C2_2026a.xlsx", templObsCatch,
  "Обліковий період — випадково обраний відрізок виборки (рекомендовано не менше 25%), а не «весь улов на око». Занижений обліковий період без пояснення в звіті про рейс — типове зауваження при перевірці.") +
sheetTemplate("templ-imaf", "11.4 Haul IMAF (взаємодія з птахами й ссавцями)", "en_C2_2026a.xlsx", templHaulImaf,
  "Записувати навіть випадки, коли тварина була відпущена живою і без видимих травм — це так само цінні дані для оцінки ефективності заходів пом'якшення.") +
sheetTemplate("templ-marinemammal", "11.5 Marine Mammal Observation", "en_C2_2026a.xlsx", templMarineMammal,
  "Заповнюється за той самий обліковий період, що й Observed Haul Catch — це дозволяє згодом зіставити частоту депредації з обсягом вилову.") +
sheetTemplate("templ-vme", "11.6 Haul VME", "en_C2_2026a.xlsx", templHaulVme,
  "Не забувати про випадкову вибірку ~30% сегментів (тип Random) на додачу до обов'язкової вибірки з ≥5 одиницями-індикаторами (тип Trigger) — це дві окремі, незалежні одна від одної вимоги.") +
sheetTemplate("templ-bio", "11.7 Biological Sampling", "en_C2_2026a.xlsx", templBioSampling,
  "Норма 7 риб/1000 гачків (35 риб на лінію, 10 з отолітами) — це мінімум, який легко пропустити при побіжному читанні інструкції; саме на це найчастіше звертають увагу під час перевірки журналів.") +
sheetTemplate("templ-conversion", "11.8 Conversion Factors", "en_C2_2026a.xlsx", templConversion,
  "Мінімум 20 риб на тест, повторювати щотижня для кожного району управління окремо — тест, зроблений один раз на весь рейс, не відповідає вимозі.") +
sheetTemplate("templ-tagging", "11.9 Tagging", "en_C2_2026a.xlsx", templTagging,
...
sheetTemplate("templ-recapture", "11.10 Tag Recapture", "en_C2_2026a.xlsx", templRecapture,
  "Фізичну мітку більше не потрібно відправляти Секретаріату поштою — досить якісного, читабельного фото на фірмовому шаблоні-лінійці CCAMLR (розділ 8.1, рис. 2). Фізичну мітку знищують на борту.") +
sheetTemplate("templ-waste", "11.11 Waste Disposal", "en_C2_2026a.xlsx", templWaste,
  "Чесне зазначення випадкових втрат знаряддя — нормативна вимога, а не привід для дисциплінарних висновків; заниження таких випадків у журналі є серйознішим порушенням, ніж сам факт втрати.") +
sheetTemplate("templ-iuu", "11.12 IUU Sightings", "en_C2_2026a.xlsx", templIuu,
  "Головне правило безпеки: фіксація, а не контакт. Спостерігач ніколи не вступає в комунікацію з підозрюваним судном самостійно — лише документує і передає інформацію капітану.");
