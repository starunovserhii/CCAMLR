...
const OLV_SCHEMA = {
  ...
  sections: [
    // -------------------------------------------------------------- 1 -----
    {
      ...
      guide: "Це «паспорт» рейсу — заповнюється один раз на початку. Блок (1) дублює дані судна з ідентифікаційних документів (IMO, назва, позивний) — вони мають повністю збігатися з тим, що зазначено у капітанському журналі C2, інакше два журнали не зіставляються. Блоки (2)–(4) відводять окремі рядки під трьох можливих спостерігачів (ID 1, 2, 3) із власними датами початку/завершення програми спостереження — це потрібно, якщо спостерігач змінюється по ходу рейсу.",
      groups: [
    ...
    // -------------------------------------------------------------- 2 -----
    {
      ...
      guide: "Головний «хребет» журналу: один рядок на кожну постановку/виборку яруса (Haul). Ліва частина (Set Details) фіксує координати початку й кінця виставлення, кількість гачків, використання освітлення палуби; права частина (Haul Details) — координати та час виборки, чи використовувався пристрій відлякування птахів, скидання відходів під час виборки.",
      tips: [{ kind: "tip", label: "Порада", text: "Якщо постановку довелося перервати (обрив яруса, шторм), обов'язково заповніть «Haul interruption duration» і опишіть ситуацію в Comment — це не помилка, а штатний спосіб задокументувати нештатну подію." }],
      fields: [
    ...
    // -------------------------------------------------------------- 3 -----
    {
      ...
      guide: "Тут фіксується улов, спостережений під час тальового періоду (частина виборки, яку спостерігач фізично оглядає рибина за рибиною). Кожен рядок — це один вид на один Haul, з розбивкою кількості за долею (залишено, викинуто, випущено, загублено на поверхні).",
      fields: [
    ...
    // -------------------------------------------------------------- 4 -----
    {
      ...
      guide: "IMAF (Incidental Mortality of seabirds And other marine Fauna) фіксує кожен випадок травмування або загибелі птаха чи морського ссавця під час постановки/виборки. Якщо за рейс жодного такого випадку не було — лист складається лише із заголовків, без жодного рядка даних.",
      tips: [{ kind: "tip", label: "Важливо", text: "Порожній лист IMAF — це не помилка заповнення, а свідчення відсутності побічної смертності. Лист не видаляють і не приховують — його наявність із самими заголовками є частиною офіційної звітності «нуль випадків»." }],
      fields: [
    ...
    // -------------------------------------------------------------- 5 -----
    {
      ...
      guide: "Цей лист призначений для фіксації самого факту спостереження морських ссавців (незалежно від того, чи була депредація) протягом тальового періоду — вид, орієнтовна кількість особин, час першого спостереження.",
      tips: [{ kind: "tip", label: "На що звернути увагу", text: "Якщо за весь рейс на цьому листі жодного разу не заповнено навіть «Observation Possible = No», варто перевірити, чи лист заповнювався системно на кожному Haul, чи був пропущений — на відміну від IMAF (де «нуль» — очікуваний результат), тут очікується хоча б позначка можливості спостереження на кожен Haul." }],
      fields: [
    ...
    // -------------------------------------------------------------- 6 -----
    {
      ...
      guide: "Застосовується у промислах, де діє Захід зі збереження CM 22-07. Ярус умовно ділиться на 20-метрові (або інші стандартні) сегменти; для кожного сегмента, де було піднято індикаторні таксони ВМЕ (губки, корали, горгонарії тощо), записується код виду, кількість одиниць у «відерці спостерігача» (Observed Bucket VME Unit(s)) та вага чи об'єм.",
      fields: [
    ...
    // -------------------------------------------------------------- 7 -----
    {
      ...
      guide: "Найдетальніший лист — індивідуальний біологічний промір кожної відібраної риби. Такі дані спочатку записуються вручну на палубі в польовий бланк (паперовий тальовий листок), а вже потім переносяться в електронний журнал — рядок за рядком, без перерахунків.",
      tips: [{ kind: "tip", label: "Практична порада", text: "Ведіть паперовий тальовий листок так, щоб порядок і нумерація риб точно відповідали Fish Serial Number у журналі — це прибирає ризик переплутати рядки при перенесенні даних увечері після довгої вахти." }],
      fields: [
    ...
    // -------------------------------------------------------------- 8 -----
    {
      ...
      guide: "При кожному успішному мічені й випуску риби записується один рядок: хто мітив (Person Tagging — Crew/Observer), координати місця випуску, довжина риби та номери обох поставлених міток (за протоколом CCAMLR риба мітиться одразу двома T-bar мітками — на випадок втрати однієї з них).",
      fields: [
    ...
    // -------------------------------------------------------------- 9 -----
    {
      ...
      guide: "Улов на борту переробляють (найчастіше — обезголовлюють і патрають, код HGT), тому для оцінки запасу потрібно перераховувати перероблену вагу назад у вагу «як є» (green weight). Для цього на вибірці риб зважують і необроблену, і оброблену тушку та обчислюють коефіцієнт (Conversion Factor = Green Weight / Processed Weight).",
      fields: [
    ...
    // ------------------------------------------------------------- 10 -----
    {
      ...
      guide: "Лист фіксує випадки, коли на борт піднімається риба, яка вже була раніше помічена (свій або чужий тег). Структура повторює мічення, але з можливими «слотами» кількох тегів (First/Second Tag), плюс додаткові біологічні проміри на момент повторної поімки.",
      tips: [{ kind: "warn", label: "Реальна помилка заповнення, знайдена в прикладі", text: "У єдиному заповненому рядку реального журналу значення були зсунуті на одну колонку праворуч відносно заголовків: у колонці «Fish Serial Number» стояв буквально текст «Fish Serial Number» замість значення, а в незадіяних колонках «Third Tag» — теж не порожньо, а текст заголовків. Це типовий артефакт копіювання рядка-шаблону в новий рядок без очищення зайвих клітинок. Практичний висновок: після заповнення рядка завжди перевіряйте, що значення дійсно потрапили під потрібні заголовки, а порожні «слоти» залишайте справді порожніми, а не з підказкою-заголовком." }],
      fields: [
    ...
    // ------------------------------------------------------------- 11 -----
    {
      ...
      guide: "Компактний лист, який заповнюється переважно раз на рейс (верхній блок — обладнання судна) плюс підсумкова таблиця по гачках і лінії яруса (скільки втрачено в морі, скільки викинуто на судні, скільки утилізовано штатно). Ці цифри використовуються для оцінки ризику «примарного» (ghost) знаряддя лову в морі.",
      groups: [
    ...
    // ------------------------------------------------------------- 12 -----
    {
      ...
      guide: "Двочастинний лист: ліва половина — для виявлених безхазяйних знарядь лову (сіті, ярус) невідомого походження, права — для самих суден, які викликають підозру в незаконному, непідзвітному або нерегульованому (ННН/IUU) промислі. Заповнюється лише за фактом події.",
      tables: [
